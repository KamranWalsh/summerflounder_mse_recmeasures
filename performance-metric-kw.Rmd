...
do_small_radar_plot <- function(metrics) {
  ...
  bounds <- tibble(mp = c("max","min"),
                   ...
                   #"total catch" = c(1,0),
                   "kept-to-released ratio" = c(1,0), #"kept:released" = c(1,0),
                   "kept per trip" = c(1,0),
                  ...
                  # "cs_per_trip" = c(1,0),
                   ...
                   "trophy" = c(1,0),
                  # "rec_removals" = c(1,0),
                   "keep one fish" = c(1,0)) #"keep_one" = c(1,0))# ,
                 #  "prop_female" = c(1,0),
                  # "ntrips" = c(1,0)) #,
...
"BTable3", "LTable3",  "STable3", "ATable3",  "BTable3.1","LTable3.1", "STable3.1", "ATable3.1", "BTable3.2","LTable3.2", "STable3.2","ATable3.2","BTable3.3","LTable3.3", "STable3.3", "ATable3.3"
...
#final paper version running from sheet 
params <- list(
  scendir = c( "~/Desktop/FlounderMSE/kw_sims_test/UnityOutput/RDMGAM/Bin3/50BBM2/sim1_bag/",
                "~/Desktop/FlounderMSE/kw_sims_test/UnityOutput/RDMGAM/Bin3/50BBM2/sim2_length/",
                "~/Desktop/FlounderMSE/kw_sims_test/UnityOutput/RDMGAM/Bin3/50BBM2/sim3_season/",
                "~/Desktop/FlounderMSE/kw_sims_test/UnityOutput/RDMGAM/Bin3/50BBM2/sim4_all/",
                "~/Desktop/FlounderMSE/kw_sims_test/UnityOutput/RDMGAM/Bin3/50BRP/sim1_bag/",
  ...
  scen.name = c(c(1:16) #these need to be consistent with whatever "run number" is in the sheet 
                ),
  fsim = rep(1,16))
...
#median(all_results$biomass, na.rm = TRUE)
#View(all_results)
```
...
median_metrics <- metrics %>% 
  group_by(om, mp, metric) %>% #if using sheet load the sheet in and run metrics2 below first, then substitute "mp" here for "hcr" before plotting 
...
median_metrics2 <- metrics %>% 
  group_by(om, RegType, metric) %>% #if using sheet load the sheet in and run metrics2 below first, then substitute "RegType" here for "reg_to_change" before plotting 
  summarize(value = median(value, na.rm=TRUE))
...
#scen_lookup <- read.csv("~/Desktop/FlounderMSE/OldVersions/scentable2.csv") %>% janitor::clean_names() %>% rename(mp = run_number)
```
...
#saveRDS(startbbmsydata_hcr, "~/Desktop/FlounderMSE/median_metrics_rdsfiles/version3_final2/startbbmsydata_hcr.rds")
#saveRDS(startbbmsydata_reg, "~/Desktop/FlounderMSE/median_metrics_rdsfiles/version3_final2/startbbmsydata_reg.rds")
```
...
metrics2 <- metrics %>% 
  ...
  mutate(om = factor(om),
      ...
      metric = fct_recode(factor(metric), 
                          "discards per trip" = "rel per trip",
                          "spawning biomass" = "spawning biomass",
                           "number of trips" = "ntrips",
                           "trophy" = "trophy",
                           "not overfishing" = "not overfishing",
                           "not overfished" = "not overfished",
                           "kept per trip" = "kept per trip",
                           "keep one fish" = "keep_one",
                           "female harvest" = "prop_female",
                           "kept-to-released ratio" = "kept:released",
                           "consumer surplus" = "cs_per_trip",
                           "recreational removals" = "rec_removals"
      ))
...
p2 <- metrics2 %>% 
 ...
 #  coord_cartesian(ylim = c(min(metrics2$ymin), max(metrics2$ymax))) + 
  ...
  guides(fill = guide_legend(nrow = 1))
# add table summaries
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics1bmsy.png",p2,width=9,height=6)
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics2bmsy.png",p2,width=9,height=6)
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics03bmsy.png",p2,width=9,height=6)

#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics1bmsy_captions.png",p2,width=9,height=6)
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics2bmsy_captions.png",p2,width=9,height=6)
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/boxplot-metrics03bmsy_captions.png",p2,width=9,height=6)
p2
```
...
#HCR
p3 <- median_metrics %>%  
  #rename(mp = hcr) %>% #only use this if running from sheet 
   mutate(om = factor(om), 
  ...
  filter(om == "Base") %>%
   ...
   mutate(metric = fct_recode(factor(metric), "kept-to-released ratio" = "kept:released", "kept per trip" = "kept per trip", "not overfished" = "not overfished", "not overfishing" = "not overfishing",  "keep one fish" = "keep_one", "trophy" = "trophy")) %>%
   do_small_radar_plot()

#REG
p3 <- median_metrics2 %>% 
   rename(mp = RegType) %>% 
   #rename(mp = reg_to_change) %>% #only use this if running from sheet 
   mutate(om = factor(om), 
        om = fct_recode(om, "Base" = "5")) %>%#,"MRIP Bias" = "2","Shift" = "3")) %>%
  filter(om == "Base") %>%
   mutate(mp = factor(mp), mp = fct_recode(mp, 
...
#here's where I don't have things updated to support entering in the sheet of input conditions
...
lab_bag <- labs(subtitle = "Bag Limit") 
lab_minlen <- labs(subtitle = "Minimum Size Limit") 
lab_seaslen <- labs(subtitle = "Season Length") 
lab_all <- labs(subtitle = "Any Regulations can Change")
...
#  mutate(type = fct_relevel(type,c("spawning biomass", "total catch"))) %>% 
 filter(type == "exp_keep", mp3 == "Minlen") 
  ...
  #minlen_proj %>% 
    ...
    #                 show.legend = FALSE) +
    scale_fill_brewer(type = "qual", palette = 2) +
    facet_wrap(~mp2, scale = "fixed") + 
    geom_line(aes(y=value,x=year,group=mp2),data = subset(minlen_proj, type != "index" & isim==1), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(minlen_proj, type != "index" & isim==2), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(minlen_proj, type != "index" & isim==3), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(minlen_proj, type != "index" & isim==4), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    ylim(0,1e7) + #originally 3000
    ylab("") + 
    xlab("") +
    theme_bw() +
...
minlen_proj_clean <- minlen_proj2 + 
...
bag_proj2 <- bag_proj %>% 
  ...
  #bag_proj %>% 
    ...
    scale_fill_brewer(type = "qual", palette = 2) +
    facet_wrap(~mp2, scale = "fixed") + 
    geom_line(aes(y=value,x=year,group=mp2),data = subset(bag_proj, type != "index" & isim==1), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(bag_proj, type != "index" & isim==2), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(bag_proj, type != "index" & isim==3), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(bag_proj, type != "index" & isim==4), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    ylim(0,1e7) + #originally 3000
    ylab("") + 
    xlab("") +
    theme_bw() +
...
seaslen_proj2 <- seaslen_proj %>% 
  ...
  #bag_proj %>% 
    ...
    scale_fill_brewer(type = "qual", palette = 2) +
    facet_wrap(~mp2, scale = "fixed") + 
    geom_line(aes(y=value,x=year,group=mp2),data = subset(seaslen_proj, type != "index" & isim==1), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(seaslen_proj, type != "index" & isim==2), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(seaslen_proj, type != "index" & isim==3), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(seaslen_proj, type != "index" & isim==4), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    ylim(0,1e7) + #originally 3000
    ylab("") + 
    xlab("") +
    theme_bw() +
...
seaslen_proj_clean <- seaslen_proj2 + 
  theme(#axis.title.y = element_blank(), 
        #axis.text.y = element_blank(), 
        #axis.title.x = element_blank(),
        #axis.text.x = element_blank()
...
allregs_proj2 <- allregs_proj %>% 
  ...
  #bag_proj %>% 
    ...
    scale_fill_brewer(type = "qual", palette = 2) +
    facet_wrap(~mp2, scale = "fixed") + 
    geom_line(aes(y=value,x=year,group=mp2),data = subset(allregs_proj, type != "index" & isim==1), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(allregs_proj, type != "index" & isim==2), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(allregs_proj, type != "index" & isim==3), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    geom_line(aes(y=value,x=year,group=mp2),data = subset(allregs_proj, type != "index" & isim==4), lty=1,lwd=0.5,col=gray(0.7),alpha=0.75) +
    ylim(0,1e7) + #originally 3000
    ylab("") + 
    xlab("") +
    theme_bw() +
...
allregs_proj_clean <- allregs_proj2 + 
  theme(axis.title.y = element_blank(), 
        axis.text.y = element_blank(), 
        #axis.title.x = element_blank(),
        #axis.text.x = element_blank()
...
#form plot
final_projection_plot <- ((bag_proj_clean + minlen_proj_clean) /
(seaslen_proj_clean + allregs_proj_clean)) + 
  plot_layout(guides = "collect") +
  plot_annotation() & 
  labs(
    y = "Expected Harvest (Numbers)", 
    x = "Year"
  )
final_projection_plot
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/projectionplot2.png",final_projection_plot,width=9,height=6)
```
...
metricperformance_all <- diag_ts %>% 
  ...
  mutate(performance = case_when(
    ...
    type == "prop_female" & value > median ~ "bad",
    type == "prop_female" & value <= median ~ "good",
    type == "rel per trip" & value > median ~ "bad",
    type == "rel per trip" & value <= median ~ "good",
    type == "rec_removals" & value > median ~ "bad",
    type == "rec_removals" & value <= median ~ "good",
    value > median  ~ "good",
    value <= median ~ "bad",
    TRUE ~ NA_character_ 
  )) %>% 
...
relpertrip <- metricperformance_all %>% filter(type == "discards per trip") %>% #filter(type == "rel per trip") %>%
...
ntrips <- metricperformance_all %>% filter(type == "number of trips") %>% #filter(type == "ntrips") %>%
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))
...
keepone <- metricperformance_all %>% filter(type == "keep one fish") %>% #filter(type == "keep_one") %>%
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))

propfemale <- metricperformance_all %>% filter(type == "female harvest") %>% #filter(type == "prop_female") %>%
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))

keptreleased <- metricperformance_all %>% filter(type == "kept-to-released ratio") %>% #filter(type == "kept:released")
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))

cspertrip <- metricperformance_all %>% filter(type == "consumer surplus") %>% #filter(type == "cs_per_trip") %>%
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))

recremovals <- metricperformance_all %>% filter(type == "recreational removals") %>% #filter(type == "rec_removals") %>%
  left_join(bagvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(seasvalues, by = c("isim", "year", "RegType", "mp2")) %>%
  left_join(minlenvalues, by = c("isim", "year", "RegType", "mp2"))
...
#bag limit 
baglimit_all <- ggplot(performancemetrics, aes(x = performance, y = bag_value, fill = factor(mp2))) +
...
baglimit_all
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_baglimit_all_caption.png",baglimit_all,width=9,height=7)

#min size limit 
minlen_all <- ggplot(performancemetrics, aes(x = performance, y = minlen_value, fill = factor(mp2))) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black", size = 0.8) +
  geom_boxplot(outlier.alpha = 0.5) +
  facet_wrap(~type, ncol = 4) + 
  theme_minimal() +
  ...
  scale_fill_manual(values = c("Biomass Matrix" = "#1b9e77", "Biological Ref. Point" = "#d95f02",
                               "No Change (RHL as target)" = "#7570b3", "Percent Change Approach" = "#e7298a"))
minlen_all
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_minlen_all_caption.png",minlen_all,width=9,height=7)

#season length limit 
seaslen_all <- ggplot(performancemetrics, aes(x = performance, y = seas_value, fill = factor(mp2))) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black", size = 0.8) +
  geom_boxplot(outlier.alpha = 0.5) +
  facet_wrap(~type, ncol = 4) + 
  theme_minimal() +
  ...
  scale_x_discrete(
   ...
   scale_fill_manual(values = c("Biomass Matrix" = "#1b9e77", "Biological Ref. Point" = "#d95f02",
                               "No Change (RHL as target)" = "#7570b3", "Percent Change Approach" = "#e7298a"))
seaslen_all
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_seaslen_all_caption.png",seaslen_all,width=9,height=7)
...
relpertrip_bag <- metricperformance_bag %>% filter(type == "discards per trip") %>% #filter(type == "rel per trip") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))
              
spawbio_bag <- metricperformance_bag %>% filter(type == "spawning biomass") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))

ntrips_bag <- metricperformance_bag %>% filter(type == "number of trips") %>% #filter(type == "ntrips") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))
...
keepone_bag <- metricperformance_bag %>% filter(type == "keep one fish") %>% #filter(type == "keep_one") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))

propfemale_bag <- metricperformance_bag %>% filter(type == "female harvest") %>% #filter(type == "prop_female") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))

keptreleased_bag <- metricperformance_bag %>% filter(type == "kept-to-released ratio") %>% #filter(type == "kept:released") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))

cspertrip_bag <- metricperformance_bag %>% filter(type == "consumer surplus") %>% #filter(type == "cs_per_trip") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))

recremovals_bag <- metricperformance_bag %>% filter(type == "recreational removals") %>% #filter(type == "rec_removals") %>%
  left_join(bagvalues_bag, by = c("isim", "year", "RegType", "mp2"))
...
#bag limit boxplot
baglimit_bag <- ggplot(performancemetrics_bag, aes(x = performance, y = bag_value, fill = factor(mp2))) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black", size = 0.8) +
  geom_boxplot(outlier.alpha = 0.5) +
  facet_wrap(~type, ncol = 4) + 
  theme_minimal() +
  ...
  scale_fill_manual(values = c("Biomass Matrix" = "#1b9e77", "Biological Ref. Point" = "#d95f02",
                               "No Change (RHL as target)" = "#7570b3", "Percent Change Approach" = "#e7298a"))
baglimit_bag
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_baglimit_bag_caption.png",baglimit_bag,width=9,height=7)
...
relpertrip_minlen <- metricperformance_minlen %>% filter(type == "discards per trip") %>% #filter(type == "rel per trip") %>%
...
ntrips_minlen <- metricperformance_minlen %>% filter(type == "number of trips") %>% #filter(type == "ntrips") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))
...
keepone_minlen <- metricperformance_minlen %>% filter(type == "keep one fish") %>% #filter(type == "keep_one") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))

propfemale_minlen <- metricperformance_minlen %>% filter(type == "female harvest") %>% #filter(type == "prop_female") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))

keptreleased_minlen <- metricperformance_minlen %>% filter(type == "kept-to-released ratio") %>% #filter(type == "kept:released") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))

cspertrip_minlen <- metricperformance_minlen %>% filter(type == "consumer surplus") %>% #filter(type == "cs_per_trip") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))

recremovals_minlen <- metricperformance_minlen %>% filter(type == "recreational removals") %>% #filter(type == "rec_removals") %>%
  left_join(minlenvalues_minlen, by = c("isim", "year", "RegType", "mp2"))
...
minlen_min <- ggplot(performancemetrics_minlen, aes(x = performance, y = minlen_value, fill = factor(mp2))) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black", size = 0.8) +
  geom_boxplot(outlier.alpha = 0.5) +
  facet_wrap(~type, ncol = 4) + 
  theme_minimal() +
  ...
  scale_fill_manual(values = c("Biomass Matrix" = "#1b9e77", "Biological Ref. Point" = "#d95f02",
                               "No Change (RHL as target)" = "#7570b3", "Percent Change Approach" = "#e7298a"))
minlen_min
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_minlen_min_caption.png",minlen_min,width=9,height=7)
...
relpertrip_seaslen <- metricperformance_seaslen %>% filter(type == "discards per trip") %>% #filter(type == "rel per trip") %>%
...
ntrips_seaslen <- metricperformance_seaslen %>% filter(type == "number of trips") %>% #filter(type == "ntrips") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))
...
keepone_seaslen <- metricperformance_seaslen %>% filter(type == "keep one fish") %>% #filter(type == "keep_one") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))

propfemale_seaslen <- metricperformance_seaslen %>% filter(type == "female harvest") %>% #filter(type == "prop_female") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))

keptreleased_seaslen <- metricperformance_seaslen %>% filter(type == "kept-to-released ratio") %>% #filter(type == "kept:released") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))

cspertrip_seaslen <- metricperformance_seaslen %>% filter(type == "consumer surplus") %>% #filter(type == "cs_per_trip") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))

recremovals_seaslen <- metricperformance_seaslen %>% filter(type == "recreational removals") %>% #filter(type == "rec_removals") %>%
  left_join(seaslenvalues_seaslen, by = c("isim", "year", "RegType", "mp2"))
...
seaslen_seas <- ggplot(performancemetrics_seaslen, aes(x = performance, y = seaslen_value, fill = factor(mp2))) +
  geom_vline(xintercept = 1.5, linetype = "dashed", color = "black", size = 0.8) +
  geom_boxplot(outlier.alpha = 0.5) +
  facet_wrap(~type, ncol = 4) + 
  theme_minimal() +
  ...
  scale_fill_manual(values = c("Biomass Matrix" = "#1b9e77", "Biological Ref. Point" = "#d95f02",
                               "No Change (RHL as target)" = "#7570b3", "Percent Change Approach" = "#e7298a"))
seaslen_seas
ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/goodbad_seaslen_seas_caption.png",seaslen_seas,width=9,height=7)
...
median_metrics22 <- readRDS("~/Desktop/FlounderMSE/median_metrics_rdsfiles/version3_final2/startbbmsydata_hcr.rds")
...
median_metrics_subset <- median_metrics22 %>% 
  ...
  #filter(metric %in% c("kept per trip", "kept:released", "ntrips", "prop_female"))
    mutate(metric = fct_recode(factor(metric), 
                           "discards per trip" = "rel per trip",
                          "spawning biomass" = "spawning biomass",
                           "number of trips" = "ntrips",
                           "trophy" = "trophy",
...
median_metrics_subset$metric <- factor(median_metrics_subset$metric, levels = c("discards per trip",
                          "spawning biomass","number of trips","trophy","not overfishing","not overfished",
                           "kept per trip","keep one fish","female harvest","kept-to-released ratio",
                           "consumer surplus","recreational removals"))
  #factor(median_metrics_subset$metric, levels = c("rel per trip", "spawning biomass", "ntrips", "trophy",
                    #"not overfishing", "not overfished", "kept per trip",
                    #"keep_one", "prop_female", "kept:released", "cs_per_trip", "rec_removals")) 

bmsy_hcr <- ggplot(median_metrics_subset, aes(mp, value, col = as.factor(StartBBMSY), 
...
bmsy_hcr
#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/bmsy_hcr_caption.png",bmsy_hcr,width=9,height=5.5)
...
median_metrics22 <- readRDS("~/Desktop/FlounderMSE/median_metrics_rdsfiles/version3_final2/startbbmsydata_reg.rds")
...
bmsy_reg <- ggplot(median_metrics_subset, aes(RegType, value, col = as.factor(StartBBMSY), 
  ...
  scale_shape_manual(values = c("Bag" = 17, "Length" = 15, "Season" = 3, "All" = 16),
                     labels = c("Bag limit (Bag)", "Minimum size limit (Length)", "Season length (Season)", "Any regulations can change (All)")) + guides( color = guide_legend(order = 1), shape = guide_legend(order = 2)) #I guess we need to manually order this one 
bmsy_reg

#ggsave("~/Desktop/FlounderMSE/ManuscriptPrep/Draft5Figures_final2/bmsy_reg.png",bmsy_reg,width=9,height=5.5)
